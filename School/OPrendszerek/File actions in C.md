functions that can be called in c like `fprint` are imported from the *stdlib* that use functions like
`write` and `open` from the **Kernel** only he can actully write to  file

# A kernel
- minden filet kent vesz eszre...aka az eger is es a keyboard is file
- 

# Argumentum default fileok 
```c
int fd
STDIN_FILENO -> 0
STDOUT_FILENO -> 1
...
# Error eseten kiiratas
```c
fprintf(stderr, "nem sikerult");
```

# Example for a cat function that uses the kernel based read and write functions
```c
/*
